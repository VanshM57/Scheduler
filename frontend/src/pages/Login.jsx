...
const Login = () => {
  ...
  const submitHandler = async (e) => {
    ...
    try{
      ...
      console.log(err);
      const errorMessage = err.response?.data?.message || "Incorrect Credentials";
    ...
    <div className="flex items-center justify-center min-h-screen bg-[#0a0a0a] relative overflow-hidden px-4 py-8">
      ...
      >
        ...
        <form className="mt-6" onSubmit={submitHandler}>
          <label className="block text-white font-medium text-sm sm:text-base">Email</label>
          <input
            className="w-full mt-2 p-2.5 sm:p-3 rounded-lg bg-white/20 text-white text-sm sm:text-base border border-transparent focus:border-blue-500 transition-all duration-300 focus:ring-2 focus:ring-blue-500"
            type="email"
            value={email}
            onChange={(e) => setEmail(e.target.value)}
            placeholder="yourname@rtu.ac.in"
            required
          />
          ...
          <button
            className="w-full mt-6 py-2.5 sm:py-3 bg-blue-600 text-white text-base sm:text-lg font-semibold rounded-lg transition-all duration-300 hover:bg-blue-700 hover:shadow-lg"
          >
            Login
          </button>
        </form>
